...
<main class="main">
    <nav aria-label="breadcrumb" class="breadcrumb-nav border-0 mb-0">
        <div class="container d-flex align-items-center">
            ...
            <ol class="breadcrumb" style="margin-left:75%;">
                <!-- Share facebook -->
                <div id="fb-root"></div>
                <script async defer crossorigin="anonymous" src="https://connect.facebook.net/vi_VN/sdk.js#xfbml=1&version=v13.0&appId=569915973958260&autoLogAppEvents=1" nonce="tETgOpud"></script>
                @foreach($attr_name1 as $key => $value2)
                <div class="fb-share-button" data-href="{{URL::to('/product-details/product_id='.$value2->product_id.'/attr_id='.$value2->attr_id)}}" data-layout="button" data-size="small"><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fdevelopers.facebook.com%2Fdocs%2Fplugins%2F&amp;src=sdkpreparse" class="fb-xfbml-parse-ignore">Chia sẻ</a></div>
                @endforeach
                    ...
                    <div class="col-md-6">
                        <div class="product-details product-details-centered">
                            <form>
                                ...
                                <input type="hidden" value="{{$value->product_price-($value->product_discount*$value->product_price/100)}}" class="cart_product_price_{{$value->product_id}}">
                                ...
                                <div class="details-filter-row details-row-size">
                                    ...
                                    ?>
                                        <a href="{{URL::to('/product-details/product_id='.$value1->product_id.'/attr_id='.$value1->attr_id)}}" type="button" style="font-size: 14px;margin-left:10px;margin-top:10px;" data-id_product="{{$value1->attr_id}}" class="btn btn-outline-dark active">{{$value1->attr_name}}</a>
                                    <?php
                                    } else {
                                    ?>
                                        <a href="{{URL::to('/product-details/product_id='.$value1->product_id.'/attr_id='.$value1->attr_id)}}" type="button" style="font-size: 14px;margin-left:10px;margin-top:10px;" data-id_product="{{$value1->attr_id}}" class="btn btn-outline-dark">{{$value1->attr_name}}</a>
                                    <?php
                                    }
                                    ?>
                                    @endforeach
                                ...
                                $qty = "select product_quantity from tbl_product where product_id = " . $value1->product_id;
                                ...
                                <div class="product-details-action">
                                    <div class="details-action-col">
                                        <div class="product-details-quantity">
                                            <input type="number" name="qty" id="qty" class="form-control" value="1" min="1" max="<?= $row1['product_quantity'] ?>" step="1" data-decimals="0" required onKeyDown="return false">
                                ...
                                <div class="product-details-footer">
                                    <div class="product-cat">
                                        ...
                                        <a href="{{URL::to('/category-product/category_id='.$value->category_id)}}">{{($value->category_name)}}</a>,
                                        @endforeach
                                        @foreach($brand as $key => $value1)
                                        <a href="{{URL::to('/brand/brand_id='.$value1->brand_id)}}">{{($value1->brand_name)}}</a>
                                        @endforeach
                                ...
                                <form action="#" style="margin-top: -35px;">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" class="form-control comment_phone" id="reply-email" pattern="([0]\d{9,})" name="reply-email" required placeholder="Số điện thoại *">
                                        </div><!-- End .col-md-6 -->
                                    </div><!-- End .row -->
